...
def main():
    ...
    if args.in_dataset == "gtsrb":
        dataset=expand_GTSRB.taggedGTSRB('./datasets/gtsrb/data', train=True, transform=transform_train, tag=args.tag)
        indices = list(range(len(dataset)))
        begin = int((len(dataset) * 0.6))
        end = int((len(dataset) * 0.8))
        np.random.shuffle(indices)
        train_sampler = SubsetRandomSampler(indices[0:begin])
        valid_sampler = SubsetRandomSampler(indices[begin:end])
        train_loader = torch.utils.data.DataLoader(dataset,batch_size=args.batch_size,**kwargs,sampler=train_sampler)
        val_loader = torch.utils.data.DataLoader(dataset,batch_size=args.batch_size,**kwargs,sampler=valid_sampler)
    else:
