...
def train(train_loader, model, criterion, optimizer, scheduler, epoch, attack_in=None):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # origin_images = torchvision.utils.make_grid(nat_input).numpy().transpose(1, 2, 0)
        # plt.imshow(origin_images)
        # plt.show()

        nat_trans_input = torch.flip(nat_input, [-1])
        ...
        if not args.adv:
            ...
            if i % args.print_freq == 0:
                print('Epoch: [{0}][{1}/{2}] loss {3:.3f} kl {4:.3f}\t'
                    ...
                    epoch, i, len(train_loader), loss2, kl2, batch_time=batch_time,
