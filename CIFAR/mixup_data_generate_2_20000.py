...
for i in range(1):
    images, labels = combine_images(sample_positions, num_classes=i + 2, num_images=count)
    print(images.shape)
    print(labels.shape)
    # exit(0)
    ...
    np.save(f'mixup_images_{count}_{i + 2}.npy', fused_images)
    np.save(f'mixup_labels_{count}_{i + 2}.npy', fused_labels)
